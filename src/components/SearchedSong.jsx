...
const SearchedSong = ({ id, name, artists, image }) => {
    const { encodeId, setSearchInput, searchInput, setSearchedSong, playAndPause, playMusic, fetchSongById } = useContext(musicContext);

    return (
        <Link to={`/${name}/search/${encodeId(id)}`} onClick={() => { setSearchInput(''), setSearchedSong([]) }} className='flex items-center gap-4 bg-[#1FCCB3] w-[46%] h-[6rem] p-2 overflow-hidden border-black border-[2px] rounded-lg cursor-pointer'>
